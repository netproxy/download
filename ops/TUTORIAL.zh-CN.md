...
### 2.0 方式 0：NPM 跨平台安装（Windows / macOS / Linux 推荐）

如果你本地或服务器已安装 Node.js（>=16），可直接通过 npm 全局安装跨平台版本的 `pushnova-ops`，或直接免安装运行：

```bash
# 全局安装（安装后终端随时可用 pushnova-ops 与 pushnova 命令）
